...
fopen: 파일 경로와 mode 인자. = 포인터 생성과 파일 염. 못열면 NULL 반환  stream=fopen("D:\\project\\data.txt","rt")
fclose() 인자: 파일 스트림을 지정 실패:EOF반환  file_state=fclose(stream)
...
---------BINARY-----------------
fread(void *buffer,size_t size, size_t count,FILE* stream) 바이너리는 따로 변환 과정 필요x 성공식 반복 횟수 반환. 실패 시 count보다 작은 값 반환 fread(&ss,sizeof(s),1,stream)
       buffer=파일로부터 입력 받을 데이터 저장하는 버퍼를 가리키는 포인터, size= 한번에 입력받을 데이터의 바이트, count=반복 횟수(size=block 개수)
fwrite(const void* buffer, size_t size, size_t count, FILE*stream)성공식 반복 횟수 반환. 실패 시 count보다 작은 값 반환.  한꺼번에 많은 양 입출력시 빛을 발함. / fwrite(buffer1, sizeof(int),5,stream)
fseek(FILE *stream,long offset, int start) 성공 0반환 실패 0 아닌 값. stream=현재 위치 파일 포인터. offset-바잍 크기. (stream,0,SEEK_SET/SEEK_CUR/SEEK_END)
ftell(FILE*stream) 성공: 파일 포인터 위치 long형으로 반환 실패:EOF  
...
int main()
{
       ...
       for (int i = 0; i < 1; i++)
       {
              ...
              scanf("%d"
                    ...
                    " %d",
                    &s[i].num,
                    s[i].name, &s[i].mid, &s[i].final, &s[i].aten, &s[i].assign);
              s[i].total = s[i].mid + s[i].final + s[i].aten + s[i].assign;
              if (s[i].total >= 95)
              {
                     s[i].grade = "A+";
              }
       ...
       for (int i = 0; i < 1; i++)
       {
              fprintf(stream, "%d"
                              ...
                              " %s\n",
                      s[i].num,
                      s[i].name, s[i].mid, s[i].final, s[i].aten, s[i].assign, s[i].total, s[i].grade);
       }
       state = fclose(stream);
       if (state == EOF)
       {
